...
@RIGOL_DG4062_serve.route('/gcs1', methods=["GET"])
def gcs1():
    result = {'name': 'RIGOL_DG4062_serve', 'cs': request.values.get('cs')}
    return jsonify(result)


@RIGOL_DG4062_serve.route('/pcs1', methods=["POST"])
def pcs1():
    result = {'name': 'RIGOL_DG4062_serve', 'cs': request.json.get('cs')}
    return jsonify(result)
